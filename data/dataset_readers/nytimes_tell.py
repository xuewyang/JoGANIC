...
@DatasetReader.register('nytimes_tell')
class NYTNewsTellWikiReader(DatasetReader):
    def __init__(self,
                 tokenizer: Tokenizer,
    ...
    def _read(self, split: str):
        ...
        with open(os.path.join(data_folder, 'images_' + split + '.json'), 'r') as j:
            img_ids = json.load(j)
        with open(os.path.join(data_folder, 'captions_' + split + '.json'), 'r') as j:
            captions = json.load(j)
        with open(os.path.join(data_folder, 'articles_' + split + '.json'), 'r') as j:
        ...
        for sample_id in ids:
            ...
            yield self.article_to_instance(article, image, caption, image_path)

    def article_to_instance(self, article, image, caption, image_path) -> Instance:
