...
from TGNC.models.decoder_tgnc_wiki import Decoder
...
class ClassificationHead(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.dense = nn.Linear(1024 + 2048 + 300, 1024)
        self.dropout = nn.Dropout(0.1)
        self.out_proj = nn.Linear(1024, 5)

    def forward(self, hidden_states, X_image, ent, ent_masks):
        hidden_states = hidden_states[:, 0, :]  # take <s> token (equiv. to [CLS])
        hidden_states = self.dropout(hidden_states)
        image_feat = X_image.mean(dim=1)
        image_feat = self.dropout(image_feat)
        ent_feat = ent.sum(dim=1) / ent_masks.sum(dim=1, keepdim=True)
        ent_feat = self.dropout(ent_feat)
        mask = torch.isinf(ent_feat).any(dim=-1)
        ent_feat[mask] = 0.
        hidden_states = torch.cat([hidden_states, image_feat, ent_feat], dim=-1)
...
@Model.register("tgnc_mstr_wiki")
class TGNCMSTRWIKIModel(Model):               # all in one model
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
    ...
    def _forward(self,
        ...
        article_ids = context[self.index + '_ms_1']
        ...
        if context[self.index + '_ms_2'].shape[1] != 0:
            article_ids_2 = context[self.index + '_ms_2']
            # article_ids.shape == [batch_size, seq_len]
            article_padding_mask_2 = article_ids_2 == 0
            X_sections_hiddens_2 = self.textmodel.extract_features(
                article_ids_2, return_all_hiddens=True)
            X_article_2 = torch.stack(X_sections_hiddens_2, dim=2)
            # X_article.shape == [batch_size, seq_len, 13, embed_size]
            X_article_00 = X_article[:, :512 - self.span - 1, :, :]
            X_article_01 = (X_article_2[:, 1:self.span+1, :, :] + X_article[:, 512 - self.span - 1:-1, :, :]) / 2.0
            X_article_02 = X_article_2[:, self.span+1:, :, :]
            X_article = torch.cat([X_article_00, X_article_01, X_article_02], dim=1)
            article_padding_mask_00 = article_padding_mask[:, :512 - self.span - 1]
            article_padding_mask_01 = article_padding_mask[:, 512 - self.span - 1:-1]
            article_padding_mask_02 = article_padding_mask_2[:, self.span+1:]
            article_padding_mask = torch.cat([article_padding_mask_00, article_padding_mask_01, article_padding_mask_02], dim=1)
        ...
        ent[ent_masks] = 0
        template_logits = self.classifier(X_article, X_image, ent, ent_masks)
