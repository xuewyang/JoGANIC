...
from transformers import LongformerModel, RobertaModel, RobertaTokenizer
...
from TGNC.models.decoder_tell import Decoder
from TGNC.models.resnet import resnet152


@Model.register("tell_longformer")
class TellLongformerModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
        ...
        self.textmodel = LongformerModel.from_pretrained('allenai/longformer-large-4096', return_dict=True,
    ...
    def _forward(self,
        ...
        global_attention_mask = torch.ones_like(article_ids)  # set local or global attention for Longformer
        global_attention_mask[:, [0]] = 2
        X_sections_hiddens = self.textmodel(article_ids, global_attention_mask=global_attention_mask)
        X_article = torch.stack(X_sections_hiddens[2], dim=2)
        X_article = X_article[:, :article_ids.shape[1], :]
